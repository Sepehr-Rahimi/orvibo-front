...
export const schemaHelper = {
  ...
  phoneNumber: (props) =>
    zod
      ...
      .min(1, {
        message: props?.message?.required_error ?? 'شماره موبایل الزامی است!',
      })
      .refine((data) => props?.isValidPhoneNumber?.(data), {
        message: props?.message?.invalid_type_error ?? 'شماره موبایل اشتباه است!',
      }),
  /**
   * date
   * defaultValue === null
  ...
  files: (props) =>
    zod.array(zod.custom()).transform((data, ctx) => {
      ...
      if (!data.length) {
        ...
        ctx.addIssue({
          code: zod.ZodIssueCode.custom,
          message: `حداقل باید ${minFiles} آیتم باشد!`,
        });
      }
