    ...
    <Stack direction="column" gap={3}>
      ...
      <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, mb: 3 }}>
        ...
        <Box component="ul" sx={{ maxHeight: 400, overflowY: 'auto' }}>
          {factorState.searchItems.map((product, index) => (
        ...
        <Table sx={{ minWidth: 720 }}>
          <TableHead>
            <TableRow>
              ...
              <TableCell aligh="center">رنگ</TableCell>
          ...
          <TableBody>
            {factorState.items.map((item, index) => (
              <TableRow key={index}>
                ...
                {/* <TableCell align="right">{item.colorName}</TableCell> */}
                <TableCell align="center">
                  <TextField
                    select
                    value={item.colorName}
                    onChange={(e) =>
                      factorState.setItems((prev) =>
                        prev.map((it, i) =>
                          i === index ? { ...it, colorName: e.target.value } : it
                        )
                      )
                    }
                    sx={{ minWidth: 120 }}
                  >
                    {item.colorsName.map((colorLabel, i) => (
                      <MenuItem key={i} value={colorLabel}>
                        <Box
                          sx={{
                            ...
                            height: 16,
                            borderRadius: '50%',
                            backgroundColor: item.colors[i] || '#ccc',
                            mr: 1,
                          }}
                        />
                        {colorLabel}
        ...
        onClick={() => {
          ...
          if (order?.id) {
            updateOrder({
              ...
              items: factorState.items.map((item) => ({
