...
export function ProductFiltersResult({ filters, totalResults, sx }) {
  ...
  const handleRemovePrice = () => {
    filters.setState({ priceRange: [0, 0] });
  };
  ...
  return (
    <FiltersResult totalResults={totalResults} onReset={filters.onResetState} sx={sx}>
      ...
      <FiltersBlock label="دسته بندی :" isShow={filters.state.category !== 'همه'}>
      ...
      >
        <Chip
          {...chipProps}
          label={`ریال ${filters.state.priceRange[0]} - ${filters.state.priceRange[1]} `}
