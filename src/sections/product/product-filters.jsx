...
export function ProductFilters({ open, onOpen, onClose, canReset, filters, options }) {
  const marksLabel = [...Array(21)].map((_, index) => {
    ...
    const firstValue = index === 0 ? `ریال ${value}` : `${value}`;
  ...
  const renderCategory = (
    <Box display="flex" flexDirection="column">
      ...
      <StyledTreeList
        ...
        selectedItems={[filters.state.category]}
      ...
      {/* {options.categories.map((option) => (
        <FormControlLabel
          ...
          label={option}
          sx={{ ...(option === 'همه' && { textTransform: 'capitalize' }) }}
        />
      ))} */}
    </Box>
  );
  ...
  const renderPrice = (
    <Box display="flex" flexDirection="column">
      ...
      <Box gap={5} display="flex" sx={{ my: 2 }}>
        <InputRange type="از" value={filters.state.priceRange} onFilters={filters.setState} />
        <InputRange type="تا" value={filters.state.priceRange} onFilters={filters.setState} />
      </Box>
